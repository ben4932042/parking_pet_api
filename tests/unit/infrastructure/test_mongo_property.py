...
@pytest.mark.asyncio
async def test_search_by_vector_uses_property_v3_vector_index():
    aggregate_cursor = AsyncMock()
    aggregate_cursor.to_list.return_value = [
        {
            "_id": "place-1",
            "place_id": "place-1",
            "name": "Cafe 1",
    ...
    items = await repo.search_by_vector(
        query_vector=[0.1, 0.2],
        limit=5,
        filters={"primary_type": "cafe"},
    )
