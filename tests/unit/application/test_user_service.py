...
async def test_update_user_profile_delegates_to_repo(user_entity_factory):
    ...
    assert repo.calls == [{"fn": "update_user_profile", "user_id": "u1", "name": "Ben Updated"}]


@pytest.mark.asyncio
async def test_update_favorite_property_delegates_to_repo(user_entity_factory):
    user = user_entity_factory(identifier="u1", name="Ben", favorite_property_ids=["p1"])
    repo = UserRepoStub(user=user)
    service = UserService(repo=repo)

    result = await service.update_favorite_property(user_id="u1", property_id="p1", is_favorite=True)
